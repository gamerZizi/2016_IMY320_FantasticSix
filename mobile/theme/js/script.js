...
$(document).ready(function(){
  /*if ($(".ui-page-active").attr("id") === "login") {
    $( "#main-menu-panel" ).css("display", "none");
    console.log("dfklgv")
  } else {
...
function signOut() {
  // Load spinning loading icon
  displayLoadingSpinner({ message: "Logging out, please wait." });
...
function loadEvents() {
  ...
  if ($("#events-loaded").val() === "true") return;
  ...
  var paramObj = {
    ...
    callback  : function(response) {
      ...
      $.each(response, function(index, obj){
        ...
        list  +=    '<li>';
        list  +=      '<a href="#event" onclick="loadEventByID('+ obj.ID +', \''+ obj.post_title.trim() +'\');" class="ui-btn">';
...
function loadEventByID(eventID, title) {
  // Load spinning loading icon
...
function loadNews() {
  if ($("#news-loaded").val() === "true") return;
  ...
  var paramObj = {
    ...
    callback  : function(response) {
      ...
      $.each(response.news, function(index, obj){
        ...
        list  +=    '<li>';
        list  +=      '<a href="#news-single" onclick="loadNewsByID('+ obj.ID +', \''+ obj.post_title.trim() +'\');" class="ui-btn">';
...
function loadNewsByID(id, title) {
  // Load spinning loading icon
  displayLoadingSpinner({ message: title });
